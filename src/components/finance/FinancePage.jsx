...
function HoldingRow({ holding, onEdit, onDelete }) {
  ...
  const [valInput, setValInput]         = useState(holding.currentValue)
  const { updateHolding } = useDataContext()
  ...
  function submitValue(e) {
    e.preventDefault()
    updateHolding(holding.id, { currentValue: Number(valInput) })
    ...
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          {holding.ticker && (
            <span className="text-xs font-bold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded font-mono">
              {holding.ticker}
            </span>
          )}
        ...
        <div className="flex items-center gap-3 mt-0.5">
          {holding.shares > 0 && (
            <span className="text-xs text-gray-400">{holding.shares} shares</span>
          )}
          {holding.costBasis > 0 && (
            <span className="text-xs text-gray-400">basis {fmtK(holding.costBasis)}</span>
          )}
        </div>
      </div>
      <div className="text-right shrink-0">
        {editingValue ? (
          <form onSubmit={submitValue} className="flex items-center gap-1">
            <input type="number" min="0" step="0.01" value={valInput}
              onChange={e => setValInput(e.target.value)} autoFocus
            ...
            <button type="button" onClick={() => setEditingValue(false)} className="text-xs text-gray-400 cursor-pointer">×</button>
          </form>
        ) : (
          <button onClick={() => { setEditingValue(true); setValInput(holding.currentValue) }}
            className="text-right cursor-pointer hover:opacity-70 transition-opacity">
            <div className="text-sm font-semibold text-gray-900 tabular-nums">{fmtK(holding.currentValue)}</div>
            {holding.costBasis > 0 && (
              <div className={`text-xs font-medium tabular-nums ${isPositive ? 'text-green-600' : 'text-red-500'}`}>
                {isPositive ? '+' : ''}{fmtK(gain)} ({gainPct.toFixed(1)}%)
              </div>
            )}
          </button>
        )}
...
export default function FinancePage() {
  ...
  const [txFilter,     setTxFilter]     = useState('all')   // all | expense | income | recurring
        ...
        {holdings.length === 0 ? (
          ...
          <div className="space-y-3">
            {INVEST_ACCOUNT_TYPES.filter(at => holdingsByAccount[at.value]).map(at => {
              ...
              const groupTotal = group.reduce((s, h) => s + (h.currentValue || 0), 0)
              const groupBasis = group.reduce((s, h) => s + (h.costBasis || 0), 0)
