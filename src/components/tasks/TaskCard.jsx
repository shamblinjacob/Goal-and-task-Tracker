...
export default function TaskCard({ task, goalTitle, onToggle, onEdit, onDelete }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-start gap-3 transition-opacity ${task.completed ? 'opacity-50' : ''}`}>
      <button
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => onEdit(task)}
              className="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 cursor-pointer transition-colors text-xs"
            >✎</button>
            <button
              onClick={() => onDelete(task.id)}
              className="p-1 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 cursor-pointer transition-colors"
            >×</button>
          </div>
        </div>
        {task.description && (
